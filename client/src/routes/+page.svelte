<svelte:head>
    <meta
        name="description"
        content="Web app for placing all your links & portfolio sharing"
    />
</svelte:head>

<section class="py-40">
    <div class="lg:container lg:mx-auto">
        <div class="text-center">
            <img src="/logo.png" alt="logo" class="m-auto w-36"/>
            <h1 class="text-7xl font-bold mt-10">Sociape</h1>
            <p class="mt-8 text-xl">One simple link to share all your media portfolio</p>
            <button class="btn btn-primary my-7 text-lg">Get started</button>
            <button class="btn btn-secondary text-lg">Example profile</button>
        </div>
    </div>
</section>